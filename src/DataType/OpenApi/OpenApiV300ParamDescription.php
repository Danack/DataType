...
class OpenApiV300ParamDescription implements ParamDescription
{
    ...
    public static function createFromInputTypes($inputTypes)
    {
        ...
        foreach ($inputTypes as $inputType) {
            $description = new self($inputType->getName());

            $firstRule = $inputType->getExtractRule();
            $firstRule->updateParamDescription($description);

            foreach ($inputType->getProcessRules() as $subsequentRules) {
...
//    description - CommonMark syntax MAY be used for rich text representation.
    ...
    public function setType(string $type): void
    {
        ...
        if (array_value_exists($knownTypes, $type) === false) {
            throw new OpenApiExceptionData("Type [$type] is not known for the OpenApi spec.");
        }
    ...
    public function setFormat(string $format): void
    {
        if ($this->type === 'number') {
            ...
            if (array_value_exists($knownFormats, $format) === false) {
                throw new OpenApiExceptionData("Format [$format] is not known for type 'number' the OpenApi spec.");
            }
        }
            ...
            if (array_value_exists($knownFormats, $format) === false) {
                throw new OpenApiExceptionData("Format [$format] is not known for type 'integer' the OpenApi spec.");
            }
        }
...
//        throw new \Exception("setCollectionFormat not implemented yet.");
    ...
    {
        if ($maxLength <= 0) {
            throw new OpenApiExceptionData("Max length must be greater than 0");
        }
    ...
    {
        if ($minLength <= 0) {
            throw new OpenApiExceptionData("Min length must be at least 0");
        }
